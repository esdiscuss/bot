...
function fix() {
  ...
  return through(function (message) {
    ...
    if (!/spam/i.test(message.header.subject) &&
      ...
      //fix subjects
      var subjectTag = convoIDs['key:' + message.header.inReplyTo] || subjects['key:' + tag(message.header.subject)] || tag(message.header.subject);
      convoIDs['key:' + message.header.messageID] = subjectTag;
      subjects['key:' + tag(message.header.subject)] = subjectTag;
      message.subjectID = subjectTag;
